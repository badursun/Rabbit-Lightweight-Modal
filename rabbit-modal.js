(function (global) {
	...
	class RabbitModal {
		...
		static #defaultOptions = {
			...
			stackable: false,
			autoShow: false, // Modalın otomatik açılması için yeni seçenek
		...
		constructor(options = {}) {
			this.#options = {
				...RabbitModal.#defaultOptions,
				...options,
			};
			RabbitModal.#instances.add(this);
			this.#debug("info", "Modal instance created", {
		...
		#createModal() {
			...
			if (
				!this.#options.type &&
				(this.#options.title || this.#options.showHeaderClose)
			) {
			...
			if (this.#options.type) {
				...
				// Alert type icons
				const icons = {
					success: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>`,
					error: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>`,
					warning: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>`,
					info: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>`,
				};
			...
			if (this.#options.buttons && this.#options.buttons.length > 0) {
				...
				this.#options.buttons.forEach((button) => {
					...
					btn.className = `rabbit-btn ${
						button.type ? `rabbit-btn-${button.type}` : ""
					} ${button.class || ""}`;
				...
				footer.addEventListener("click", (e) => {
					...
					const buttonConfig = this.#options.buttons.find(
						(btn) => btn.key === action
					);
					if (!buttonConfig) {
		...
		#setupEventHandlers() {
			...
			closeButtons.forEach((button) => {
				...
				this.#eventListeners["closeButtons"].push({
					element: button,
		...
		#updateSize() {
			...
			const size = validSizes.includes(this.#options.size)
				? this.#options.size
				: "default";
		...
		show() {
			...
			// Timer'ı başlat
			if (
				this.#options.timer?.enabled &&
				this.#options.timer?.duration > 0
			) {
				this.startTimer();
			}
		...
		destroy() {
			...
			if (this.#eventListeners) {
				...
				if (this.#eventListeners["overlay"]) {
					const overlay =
						this.#element?.querySelector(".rabbit-overlay");
					if (overlay) {
						overlay.removeEventListener(
							"click",
							this.#eventListeners["overlay"]
						);
					}
				}
				...
				if (this.#eventListeners["closeButtons"]) {
					this.#eventListeners["closeButtons"].forEach(
						({ element, handler }) => {
		...
		static #getVisibleModals() {
			return Array.from(RabbitModal.#instances).filter(
		...
		async #startAjaxRequest() {
			...
			try {
				...
				if (this.#options.ajax.onStart) {
					this.#options.ajax.onStart(
						this,
						this.#element.querySelector(".rabbit-modal-content")
					);
				}
				...
				if (this.#options.ajax.delay) {
					this.#debug("info", "AJAX request delayed", {
						delay: this.#options.ajax.delay,
					});
					await new Promise((resolve) =>
						setTimeout(resolve, this.#options.ajax.delay)
					);
				}
				...
				const fetchPromise = fetch(this.#options.ajax.url, {
					...
					body: this.#options.ajax.data
						? JSON.stringify(this.#options.ajax.data)
						: null,
					signal: this.#abortController.signal,
					credentials:
						this.#options.ajax.credentials || "same-origin",
				});
				...
				this.#debug("success", "AJAX request successful", {
					data,
				});

				// onSuccess callback
				if (this.#options.ajax.onSuccess) {
					this.#options.ajax.onSuccess(
						this,
						data,
						this.#element.querySelector(".rabbit-modal-content")
					);
				...
				// Retry mekanizması
				if (
					this.#options.ajax.retry?.enabled &&
					(!this.#retryCount ||
						this.#retryCount < this.#options.ajax.retry.limit)
				) {
					this.#retryCount = (this.#retryCount || 0) + 1;

					this.#debug(
						"warn",
						"AJAX request failed, initiating retry",
						{
							error: error.message,
							...
							nextAttemptIn: `${
								this.#options.ajax.retry.delay
							}ms`,
						}
					);

					await new Promise((resolve) =>
						setTimeout(resolve, this.#options.ajax.retry.delay)
					);
				...
				this.#debug("error", "AJAX request failed", {
					error,
				});

				// onError callback
				if (this.#options.ajax.onError) {
					this.#options.ajax.onError(
						this,
						error,
						this.#element.querySelector(".rabbit-modal-content")
					);
		...
		#updateContent(data) {
			const content =
				this.#options.ajax.contentType === "html"
					? data
					: typeof data === "string"
					? data
